...
namespace boost {
    namespace fibers {
        namespace algo {
            ...
            template <int SLOT>
            void
                exclusive_shared_work<SLOT>::awakened(context* ctx) noexcept {
            ...
            template <int SLOT>
            context*
                exclusive_shared_work<SLOT>::pick_next() noexcept {
                context* ctx = nullptr;
                std::unique_lock< std::mutex > lk{ rqueue_mtx_ };
                if (!rqueue_.empty()) { /*<
            ...
            template <int SLOT>
            void
                exclusive_shared_work<SLOT>::suspend_until(std::chrono::steady_clock::time_point const& time_point) noexcept {
                if (suspend_) {
            ...
            template <int SLOT>
            void
                exclusive_shared_work<SLOT>::notify() noexcept {
                if (suspend_) {
                    std::unique_lock< std::mutex > lk{ mtx_ };
                    flag_ = true;
                    lk.unlock();
            ...
            template <int SLOT>
            typename exclusive_shared_work<SLOT>::rqueue_type exclusive_shared_work<SLOT>::rqueue_{};

            template <int SLOT>
            std::mutex exclusive_shared_work<SLOT>::rqueue_mtx_{};
